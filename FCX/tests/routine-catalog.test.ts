...
describe("FCX routine catalog", () => {
  ...
  it("requests uncached JSON once with a four-second timeout", async () => {
    ...
    await expect(requestRoutineCatalog(request, { now: () => 123 }))
      .resolves.toMatchObject({ catalogVersion: 50 });
  ...
  it("preserves user overrides and custom flows while replacing session builtins", async () => {
    ...
    expect(store.get(custom.id)?.name).toBe("我的自定义");
    expect(taskClone.name).toBe("5×80+ → 后续升级");

    const reintroduced = structuredClone(catalog(51));
    ...
    expect(store.get("totw-x5")?.name).toBe("我的覆盖");
