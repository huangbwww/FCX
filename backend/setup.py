...
def preprocess_data(df: pd.DataFrame, sbc):
    persist_debug = os.getenv("FCX_SOLVER_PERSIST_DEBUG", "0") == "1"
    if persist_debug:
...
def runAutoSBC(sbc, players, maxSolveTime):
    ...
    final_players, status, status_code, rating_optimization = optimize.SBC(
        df, sbc, maxSolveTime
    )
    rating_requirements = [
        req
        for req in sbc["constraints"]
        if req.get("requirementKey") == "TEAM_RATING"
        and req.get("scope") in {"GREATER", "EXACT"}
        and req.get("eligibilityValues")
    ]
    rating_window = None
    if rating_requirements:
        target_rating = max(
            float(req["eligibilityValues"][0]) for req in rating_requirements
        )
    ...
    if final_players:
        ...
        if rating_window is not None:
            add_log(
                ...
                f"方案评分 {squad_rating:.2f}，"
                f"总成本 {df_out['price'].sum():,.0f}"
            )
        df_out["Org_Row_ID"] = df_out["Original_Idx"] + 2
        df_out.pop("Original_Idx")
        if os.getenv("FCX_SOLVER_PERSIST_DEBUG", "0") == "1":
        ...
        json_compatible_item_data = jsonable_encoder(
            {
                ...
                **(
                    {"rating_optimization": rating_optimization}
                    if rating_optimization is not None
                    else {}
                ),
            }
        )
        return JSONResponse(content=json_compatible_item_data)
    if status_code == 3 and rating_window is not None:
        add_log(
            "无法在 "
            f"{rating_window['minimum']:.2f}–{rating_window['maximum']:.2f} "
            "内完成该 SBC，请调整球员范围、保护或排除设置。"
        )
    if (
        rating_optimization
            ...
            **(
                {
                    "rating_optimization": rating_optimization,
                    **(
                        {"failure_code": rating_optimization["failure_code"]}
                        if rating_optimization.get("failure_code")
                        else {}
                    ),
                }
                if rating_optimization is not None
...
def calc_squad_rating(ratings):
    return optimize.calculate_squad_rating(ratings)
